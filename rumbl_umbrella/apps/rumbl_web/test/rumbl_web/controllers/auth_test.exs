defmodule RumblWeb.AuthTest do
  ...
  test "login with a valid username and pass", %{conn: conn} do
    ...
    {:ok, conn} = Auth.login_email_pass(conn, "me@test", "secret")

    assert conn.assigns.current_user.id == user.id
  end

  test "login with a not found user", %{conn: conn} do
    assert {:error, :not_found, _conn} = Auth.login_email_pass(conn, "me@test", "secret")
  end

  test "login with password mismatch", %{conn: conn} do
    _ = user_fixture(username: "me", email: "me@test", password: "secret")

    assert {:error, :unauthorized, _conn} = Auth.login_email_pass(conn, "me@test", "wrong")
  end
end
