defmodule Rumbl.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "requires passwords to be at least 6 characters long" do
  ...
  describe "authenticate_by_email_and_pass/2" do
    ...
    test "returns user with correct password", %{user: %User{id: id}} do
      assert {:ok, %User{id: ^id}} = Accounts.authenticate_email_pass(@email, @pass)
    end

    test "returns unauthorized error with invalid password" do
      assert {:error, :unauthorized} = Accounts.authenticate_email_pass(@email, "badpass")
    end

    test "returns not found error with no matching user for email" do
      assert {:error, not_found} = Accounts.authenticate_email_pass("bademail@localhost", @pass)
    end
  end
